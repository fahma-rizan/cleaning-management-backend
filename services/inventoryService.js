...
const getItemById = async (itemId) => {
  const item = await InventoryItem.findOne({ _id: itemId, isActive: true });
...
const adjustStock = async (itemId, { type, quantity, reference, notes, performedBy }) => {
  ...
  await session.withTransaction(async () => {
    const item = await InventoryItem.findOne({ _id: itemId, isActive: true }).session(session);
    if (!item) throw new Error('Item not found');
    ...
    await InventoryTransaction.create(
      [{
        itemId, type, quantity, quantityBefore, quantityAfter,
    ...
    result = { item, isLowStock: quantityAfter <= item.lowStockThreshold };
  });
...
const getLowStockItems = async () => {
  const items = await InventoryItem.find({ isActive: true }).lean({ virtuals: true });
...
const createBottlesOnRestock = async (itemId, quantity, nominalCapacity, restockTxnId) => {
  ...
  if (!item) throw new Error('Item not found');
  if (item.type !== INVENTORY.ITEM_TYPES.CONSUMABLE) {
    throw new Error('Bottle tracking only applies to consumable items');
  }
