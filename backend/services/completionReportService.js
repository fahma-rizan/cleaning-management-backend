 ...
 */
const submitReport = async (bookingId, submittedBy, { consumables }) => {
 ...
 *
 * On approval: returned stock is added back.
 * On rejection: the report is unlocked so the job lead can revise and resubmit.
 * Loyalty points are NOT handled here — they are awarded in bookingService
 * when admin marks the booking status as completed.
 */
...
const getEmployeeReportHistory = async (userId) => {
  return CompletionReport.find({ submittedBy: userId })
    .sort({ createdAt: -1 })
    .select('bookingRef status anomalyFlags consumables createdAt');
};
