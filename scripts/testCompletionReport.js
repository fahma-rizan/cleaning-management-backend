...
mongoose.connect(process.env.MONGO_URI).then(async () => {
  const { Booking, User, MaterialRequest, InventoryItem, Equipment } = require('../models/index');
  ...
  const vacuum = await Equipment.findOne({ name: 'Vacuum cleaner', status: 'available' });
  console.log('8. Vacuum available again:', vacuum ? 'YES' : 'NO');

  const finalBooking = await Booking.findById(booking._id);
  console.log('9. Final materialStatus:', finalBooking.materialStatus);
